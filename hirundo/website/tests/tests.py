...
class ServicesTestCase(TestCase):
    def setUp(self):
        ...
        self.test_user = User.objects.create_user(
            ...
            self.email,
            self.password)
        self.test_user2 = User.objects.create_user("az", "az@abv.bg", "456")
        self.test_user3 = User.objects.create_user("ti", "ti@abv.bg", "abv")
        ...
        UserFollowingRelationship.objects.create(
            follower=self.test_user3,
            followed=self.test_user)
    ...
    def test_login_valid_user(self):
        response = self.client.post('/login/', {'username' : 'test', 'password:' : '123'})
    ...
    def test_users(self):
        ...
        self.assertTrue(self.test_user2.username in str(response.context['not_followed_users']))
        self.assertFalse(self.test_user2.username not in str(response.context['not_followed_users']))

    def test_follow_user(self):
        self.client.login(username='test', password='123')
        self.client.get('/follow/az')
        self.assertTrue(UserFollowingRelationship.objects.filter(
            follower=self.test_user,
            followed=self.test_user2).exists())
        self.assertFalse(UserFollowingRelationship.objects.filter(
            follower=self.test_user,
            followed=self.test_user3).exists())

    def test_unfollow_user(self):
        self.client.login(username="ti", password="abv")
        self.assertTrue(UserFollowingRelationship.objects.filter(
            follower=self.test_user3,
            followed=self.test_user).exists())
    ...
    def test_messages(self):
        ...
        self.assertTrue(self.message1 in response.context['all_messages_from_followed_users'])
        self.assertTrue(self.message2 in response.context['all_messages_from_followed_users'])
        self.assertFalse(self.message3 in response.context['all_messages_from_followed_users'])
