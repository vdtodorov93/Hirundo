...
def register(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            if pass1 != pass2 or pass1 is None:
                pass_error = True
...
def users(request):
    ...
    followed_users = [
        usr.followed for usr in
        UserFollowingRelationship.objects.all().filter(
...
def follow(request, follow_user):
    if not UserFollowingRelationship.objects.filter(
        follower__username=request.user.username,
        followed__username=follow_user
    ).exists():
        followed_user = User.objects.all().filter(username=follow_user).first()
        follow_relationship = UserFollowingRelationship(
...
def unfollow(request, unfollow_user):
    try:
        UserFollowingRelationship.objects.filter(
            follower__username=request.user.username,
            followed__username=unfollow_user
        ).delete()
    except:
        pass
    return redirect('/users')
...
def messages(request):
    followed_users = [
        user.followed.username for user in
        UserFollowingRelationship.objects.filter(
            follower__username=request.user.username
        ).all()
    ]
    all_messages_from_followed_users = Message.objects.filter(
        author__username__in=followed_users
